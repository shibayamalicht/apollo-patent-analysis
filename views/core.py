...
# ==================================================================
# --- ヘルパー関数 & リソースロード（モジュールレベル・ロジック無改変） ---
# ==================================================================
@st.cache_resource
def load_tokenizer_core(): return Tokenizer()
t = load_tokenizer_core()

# ストップワード。旧ページはスクリプト先頭で毎回評価していたため、
# render() 冒頭の _refresh_stopwords() で同じ挙動を再現する。
STOP_WORDS = patiroha.get_stopwords()


def _refresh_stopwords():
    """session_state のストップワード設定を最新化する（毎描画・旧ページと同じ挙動）"""
    global STOP_WORDS
...
# 検索対象テキストから取り除く記号。論理式のリテラル照合（_core_literal_regex）でも
# 同じ集合を使うため、ここ1か所で定義する（片方だけ変えると分類コードが一致しなくなる）
_CORE_SEP_CHARS = r'!！?"“”#$%＆&\'()（）*＋+,\-．.\:：;；<=>?？@\[\]［］\\^_`{|}~〜〜／/'
_CORE_SEP_CLASS = r'[\s' + _CORE_SEP_CHARS + r']'
...
def _core_text_preprocessor(text):
    ...
    text = re.sub('[' + _CORE_SEP_CHARS + ']', ' ', text)
    return text
...
# --- 論理式検索エンジン（再帰下降パーサ。旧 CORE 実装のまま） ---
def _core_literal_regex(norm):
    """論理式のリテラル1語を、表記ゆれに強い正規表現へ変換する。

    検索対象テキストは _core_text_preprocessor で記号（'/' 等）が空白へ
    置き換わる（'H02J3/00' → 'h02j3 00'）。リテラル側の記号をそのまま照合すると
    分類コードが1件も一致しないため、記号・空白は「あってもなくてもよい区切り」
    として扱う。さらに分類コードらしい並びは、サブクラスとグループの境目でも
    区切りを許し、'H02J3/00'（詰めた表記）と 'H02J 3/00'（IPC 公式表記）の
    どちらで書いてもデータ側の両表記に一致させる。
    """
    parts = [p for p in re.split(_CORE_SEP_CLASS + '+', norm) if p]
    if not parts:
        return re.escape(norm)
    # 例: 'h02j3', '00' → 'h02j300' が分類コードの並びなら英字/数字の境目でも区切る
    if re.fullmatch(r'[a-z]\d{2}[a-z][0-9a-z]*', ''.join(parts)):
        parts = [m.group() for p in parts for m in re.finditer(r'[a-z]+|\d+', p)]
    return (_CORE_SEP_CLASS + '*').join(re.escape(p) for p in parts)
...
class RegexNode(LogicNode):
    def __init__(self, pattern):
        # to_regex_string で部分式を再帰的に正規表現へ戻すため、元パターン文字列も保持する
        self.source = pattern
...
class CoreLogicParser:
    ...
    # 分類コード（IPC / FI）はサブクラスとグループの間に半角スペースを置く公式表記がある
    # （例: 'H10B 41/27'）。空白で切ると 'H10B' と '41' に分かれて構文エラーになるため、
    # サブクラス+空白+グループ番号の並びは1つのリテラルとして先に拾う。
    CODE_TOKEN = r'[A-Za-z]\d{2}[A-Za-z][ 　]+\d+(?:[/／][0-9A-Za-z,]+)?'

    def tokenize(self, rule_str):
        # 字句解析: ( ) + * nearN adjN 分類コード またはリテラルへ分割
        raw_tokens = re.findall(
            r'\(|\)|' r'\bnear\d+\b|' r'\badj\d+\b|' + self.CODE_TOKEN + r'|' r'[\+\*]' r'|' r'[^\(\)\+\*\s]+',
    ...
    def parse(self, rule_str):
        ...
        if self.pos < len(self.tokens):
            raise ValueError(f"式の末尾に解釈できないトークンがあります: {self.tokens[self.pos]}")
        return node
    ...
    def factor(self):
        ...
        # (A+B) near C を支えるため、部分式を正規表現文字列へ変換できる場合のみ許可する。
        # 制約: near/adj の対象は「正規表現化可能」なノード（リテラル or リテラルのOR）のみ。
        #       near/adj の内側に AND は置けない。
        left = self.atom()
        ...
        while self.pos < len(self.tokens) and re.match(r'^(near|adj)\d+$', self.tokens[self.pos], re.IGNORECASE):
            ...
            # 再帰的制約チェック: 左右とも正規表現文字列に変換可能であること
    ...
    def atom(self):
        if self.pos < len(self.tokens) and self.tokens[self.pos] == '(':
            ...
            if self.pos < len(self.tokens) and self.tokens[self.pos] == ')':
                ...
                raise ValueError("閉じ括弧 ) が不足しています")
        elif self.pos < len(self.tokens):
            t = self.tokens[self.pos]
            self.pos += 1
            # リテラル（表記ゆれの吸収は _core_literal_regex に集約）
            norm = unicodedata.normalize('NFKC', t).lower()
            return RegexNode(_core_literal_regex(norm))
        else:
            raise ValueError("式が途中で終わっています")

    def to_regex_string(self, node):
        # ノードを正規表現文字列へ戻すヘルパー（OR / リテラルのみ許可）
        if isinstance(node, RegexNode):
            # RegexNode は元パターン文字列を保持している
        ...
        if isinstance(node, AndNode):
            raise ValueError("NEAR/ADJ の内側に AND (*) は使えません。OR (+) のみ使用できます。")
        return ""
...
def core_rule_error(rule_str):
    """論理式が解釈できない理由を日本語で返す（解釈できるなら None）。

    「解釈できません」だけでは、長い論理式のどこが悪いのか分からないため、
    失敗位置のトークンと、よくある原因への対処を添えて返す。
    """
    try:
        CoreLogicParser().parse(rule_str)
        return None
    except Exception as e:
        msg = str(e)
        parser = CoreLogicParser()
        try:
            parser.tokenize(rule_str)
        except Exception:
            return msg
        # 演算子なしで語が並んでいる箇所を探して具体的に示す
        toks = parser.tokens
        for i in range(len(toks) - 1):
            a, b = toks[i], toks[i + 1]
            if a not in '()+*' and b not in '()+*' \
                    and not re.match(r'^(near|adj)\d+$', a, re.IGNORECASE) \
                    and not re.match(r'^(near|adj)\d+$', b, re.IGNORECASE):
                return (f"{msg}\n\n"
                        f"「{a} {b}」のように語と語が空白で並んでいます。"
                        f"半角スペースは語の区切りとして扱われるため、間に演算子が必要です"
                        f"（OR なら `{a} + {b}`、AND なら `{a} * {b}`）。"
                        f"分類コードを書く場合は `H10B 41/27` のようにコード1つを続けて書いてください。")
        return msg
...
# ==================================================================
# --- クラスタリング高度化（数理的土台・最適k・SBERT・HDBSCAN） ---
# ==================================================================
# 設計方針（数理情報学）:
...
def core_prepare_features(texts, feature_mode, embeddings=None, min_df=2, svd_components=80):
    ...
    feature_mode: 'tfidf' / 'sbert'（sbert はMission Control の前処理で計算済みの埋め込みを再利用）。"""
...
def core_cluster(X, method='kmeans', k=8, min_cluster_size=15,
    ...
    なりやすいため、他モジュールと同様に UMAP で低次元化してから HDBSCAN する
...
def _import_rules_json(json_text):
    """AIが返した分類ルールJSONを core_classification_rules に登録する共通処理。
    ①のAIキット（貼り戻し）と ②のJSON一括インポートの両方から使う。
    戻り値: 登録した件数（失敗時は st.error を表示して None）。
    """
    try:
        cleaned_json = re.sub(r'^```json\s*|\s*```$', '', str(json_text).strip(), flags=re.MULTILINE)
        ...
        st.error(f"AIの回答を読み取れませんでした（JSON形式か確認してください）: {e}")
        return None

def render():
    # スナップショットのモジュール名判定に使用（キー・値とも旧版から不変）
    st.session_state['current_page'] = 'CORE'
    _refresh_stopwords()

    apollo_ui.page_header(
        "ルール分類", "CORE",
        "論理式（AND / OR / NEAR / ADJ）による分類と、2軸クロス集計によるポートフォリオ構造の分析")
    apollo_ui.require_data()
    st.caption("はじめての方は **① AIと分類ルールを作る → ④ 結果を見る** の2ステップです。ルールの微調整は②、やり直しは③でできます。")
    ...
    # ==============================================================
    # 分類コード（IPC/FI）検索オプション（①のキット実行と③の実行の両方で使う共通UI。
    # ①と③は排他表示のため同じウィジェットキーで衝突しない）
    # ==============================================================
    def _render_ipc_search_checkbox():
        _ipc_assigned = any(col_map.get(_ck) and col_map.get(_ck) in df_main.columns
                            for _ck in ('ipc', 'fi'))
        # 既定: 要約なしデータでは分類コード検索をONにして名称だけの検索を補う
        st.session_state.setdefault('core_search_include_ipc', _ipc_assigned and not col_map.get('abstract'))
        st.checkbox(
            "分類コード（IPC/FI）も検索対象に含める", key='core_search_include_ipc',
            disabled=not _ipc_assigned,
            help="ONにすると、分類ルールの語が分類コードの列にも一致するようになり、"
                 "`H02J3/00 + H02J13` のようなコード指定のルールが使えます（部分一致。メイングループ指定で配下の展開記号にも一致）。"
                 "コードの指定には AND(*)・OR(+) を使ってください（near/adj はコード列には不向きです）。"
                 + ("" if _ipc_assigned else "　※「Mission Control」で分類コード列を割り当てると使えます。"))
        return _ipc_assigned

    # ==============================================================
    # 分類の実行（共通コードパス）
    # フェーズ③の「すべての分類を実行」ボタンと、①のAIキット「登録して分類まで実行」の
    # 両方から呼ばれる。中身は旧フェーズ③のボタン処理そのまま
    # （分類ロジック・session_state キー・CAPCOM 保存とも無改変。関数に切り出しただけ）。
    # ==============================================================
    def _core_run_classification():
        with st.spinner("実行中..."):
            try:
                df_res = df_main.copy()
                ...
                # 分類コード（IPC/FI）も検索対象に含める（オプション・部分一致でコード指定が効く。
                # IPC・FIの両方が割り当てられていれば両方の列を検索する）
                if st.session_state.get('core_search_include_ipc'):
                    for _ck in ('ipc', 'fi'):
                        if col_map.get(_ck) and col_map[_ck] in df_res.columns:
                            search_cols.append(df_res[col_map[_ck]].fillna(''))
                ...
                for ax, cats in rules.items():
                    ...
                    for cn, cd in cats.items():
                        ...
                        # 論理式を解析（失敗したカテゴリはスキップして警告）
                        node = parse_core_rule(r_str)
                        if node:
                            compiled_rule_nodes[ax].append((cn, node))
                        else:
                            _why = core_rule_error(r_str)
                            st.warning(
                                f"**{ax} / {cn}** の論理式を解釈できなかったためスキップしました。\n\n"
                                f"{_why or ''}\n\n"
                                f"```\n{r_str}\n```",
                ...
                st.success("完了！ 結果は「④ 特許マップ作成」でクロス集計できます。")
                ...
                # CAPCOM: patents.csv更新（分類列を追加）
                try:
                    import capcom
                    if capcom.is_active():
                        capcom.save_patents_csv()
                except Exception as e:
                    st.caption(f":material/warning: WARN 分類結果(patents.csv) の CAPCOM 保存に失敗しました（要確認）: {e}")
    ...
    current_phase = st.radio(
        "フェーズ選択:",
        ["① AIと分類ルールを作る", "② ルールを手直しする", "③ 分類をやり直す", "④ 結果を見る（特許マップ）"],
        horizontal=True, key="core_phase_selector",
        help="ルール分類の作業段階を切り替えます。基本の流れは ①（AIとルールを作って分類まで実行）→ ④（結果をマップで見る）の2つ。②＝登録済みルールの手直し、③＝手直し後の分類のやり直し、に使います。")
    st.markdown("---")

    # --- ① 分類ルールの設計（AIアシスト） ---
    if current_phase.startswith("①"):
        st.subheader("① AIと分類ルールを作る — チャットAI用キット")
        _kit_no_abstract = not col_map.get('abstract')
        if _kit_no_abstract:
            st.caption(
                "あなたの母集団に合った分類ルール一式を、3ステップでAIに設計してもらいます。"
                "手順は3つです — 依頼文を生成 → チャットAIに貼付 → 回答を貼り戻す。")
            st.info(
                "このデータには要約がないため、AIには **FI（技術分類コード）ベースの「技術分類」軸** の設計を依頼します。"
                "課題・解決手段のような内容ベースの分類軸を作るには、要約ありのデータが必要です。")
        else:
            st.caption(
                "あなたの母集団に合った分類ルール一式（技術×課題×解決手段）を、3ステップでAIに設計してもらいます。"
                "手順は3つです — 依頼文を生成 → チャットAIに貼付 → 回答を貼り戻す。")

        # ---- おすすめ実行バー（設定は触らなくても動く） ----
        with st.container(border=True):
            _kb1, _kb2 = st.columns([2.6, 1], vertical_alignment="center")
            with _kb1:
                if _kit_no_abstract:
                    st.markdown(":material/smart_toy: **おすすめ設定でそのまま作れます** — 発明の名称を意味ベクトルで自動グループ分けし、FI頻出コードを添えたFIベースの分類設計の依頼文を生成します")
                else:
                    st.markdown(":material/smart_toy: **おすすめ設定でそのまま作れます** — 要約テキストを意味ベクトルで自動グループ分けし、各テーマの代表特許を添えた依頼文を生成します")
                st.caption("グループの数は母集団の規模から自動設定されます。分類の細かさなどを変えたいときだけ、下の「詳細設定」を開いてください。")
            with _kb2:
                _kit_go = st.button(":material/counter_1: AIへの依頼文を作る", type="primary", key="core_run_ai", use_container_width=True)

        # 分類コード検索の設定は依頼文の内容にも反映されるため、生成ボタンより前に見せる
        _render_ipc_search_checkbox()
        if not _kit_no_abstract and st.session_state.get('core_search_include_ipc'):
            st.caption(":material/lightbulb: ONのため、依頼文にも「分類コードを論理式に使ってよい」旨と、代表文献ごとの分類コード・母集団の頻出コードが含まれます。")

        _adv = st.expander("詳細設定（対象カラム・クラスタリング・分類軸の数）", expanded=False)
        with _adv:
            # 既定は「要約」: 日本特許の要約は【課題】【解決手段】の記載を含むことが多く、
            # 技術×課題×解決手段の3軸設計に最も効く証拠テキストになる（名称だけでは課題軸が作れない）
            col_map_options = [col_map[k] for k in ['abstract', 'title', 'claim'] if col_map.get(k)]
            target_column = st.selectbox(
                "分析対象カラム:", options=col_map_options, key="core_target_col",
                help="AIに見せる代表文献のテキスト列です。既定の「要約」は課題・解決手段の記載を含むため、"
                     "3つの分類軸（技術・課題・解決手段）の設計材料として最も効果的です。名称は短く課題情報が乏しく、"
                     "請求項は法律文体で冗長になりがちです。")
            ...
            with fc1:
                # 既定はSBERT: 「前処理」で計算済みの意味ベクトルを再利用（高速）でき、
                # テーマ単位のまとまりが良い＝AIに見せる代表サンプルの質が上がる
                _feat_opts = (["AI意味解析（SBERT）"] if _emb_ok else []) + ["特徴語抽出（TF-IDF）"]
                feature_label = st.radio("特徴量:", _feat_opts, key="core_feature_mode",
                    help="クラスタリングに使う特徴ベクトルの種類です。SBERT＝文章の意味ベクトル（言い回しが違っても意味が近ければ近接・推奨）、TF-IDF＝単語の出現頻度ベース（語の一致を重視）。意味的なまとまりはSBERT、語の一致重視はTF-IDFが得意です。")
                feature_mode = 'sbert' if 'SBERT' in feature_label else 'tfidf'
                if not _emb_ok:
                    st.caption("※AI意味解析（SBERT）を使うには「Mission Control」で前処理を実行してください。")
            with fc2:
                method_label = st.radio("クラスタ手法:", ["指定した数に分ける（KMeans・最適k自動）", "数を自動で決める（HDBSCAN）"], key="core_cluster_method",
                    help="KMeans＝指定した数(k)に必ず分割し全件どこかに所属、HDBSCAN＝密度ベースで数を自動決定し外れ値はノイズとして分離。数を固定したいならKMeans、外れ値を分けたい・数を決めたくないならHDBSCAN。")
                cluster_method = 'hdbscan' if 'HDBSCAN' in method_label else 'kmeans'
            with fc3:
            ...
            if cluster_method == 'kmeans':
                ...
                if st.button("最適クラスタ数を分析（任意・推奨kを自動入力）", key="core_run_optk"):
                ...
                # 既定kは母集団規模から自動決定（8固定だと大規模母集団で粗すぎる）
                _default_k = utils.suggest_target_k(len(df_main))
                if _scores is not None and not _scores.empty and _fresh:
                    ...
                    st.success(f"推奨 k = **{_reck}**（分類品質スコア最大）　｜　特徴量: {_info.get('mode','tfidf')} / {_dim}")
                    _figk = go.Figure()
                    _figk.add_trace(go.Scatter(x=_scores['k'], y=_scores['silhouette'],
                        name='分類品質スコア（高い=良）', mode='lines+markers', yaxis='y1'))
                    _figk.add_trace(go.Scatter(x=_scores['k'], y=_scores['Davies-Bouldin'],
                        name='クラスタの重なり度（低い=良）', mode='lines+markers', yaxis='y2', line=dict(dash='dot')))
                    ...
                    _figk.update_layout(height=300, margin=dict(t=30, b=10, l=10, r=10),
                        yaxis=dict(title='分類品質スコア'),
                        yaxis2=dict(title='クラスタの重なり度', overlaying='y', side='right'),
                        xaxis=dict(title='クラスタ数 k', dtick=1), legend=dict(orientation='h'))
                    st.plotly_chart(_figk, use_container_width=True)
                    apollo_ui.howto(
                        "クラスタ数kを変えながら、分け方の良さを採点した結果です。"
                        "<b>実線（分類品質スコア）が最も高くなるk</b>（赤い縦線の位置）がおすすめの分割数で、下のスライダに自動反映されます。"
                        "スコアはあくまで目安なので、<b>粗すぎ／細かすぎと感じたら±2程度の範囲で調整</b>して構いません。"
                        "※専門指標では実線=silhouette係数、点線=Davies-Bouldin指標です。",
                        title="このグラフの見方",
                    )
                    with st.expander("分類品質スコアの内訳（専門指標: silhouette / Calinski-Harabasz / Davies-Bouldin）"):
                        st.caption("silhouette＝クラスタのまとまりの良さ（高い=良）／ Calinski-Harabasz＝クラスタ間の分離度（高い=良）／ Davies-Bouldin＝クラスタどうしの重なり（低い=良）")
                ...
                # スライダは session_state 駆動（手順1 が推奨kを書き込む）。範囲外をクランプしてから生成
                ...
                chosen_k = utils.suggest_target_k(len(df_main))  # KMeansフォールバック時のk
                core_hdbscan_auto = st.checkbox(
                    ":material/smart_toy: 自動最適化（最小クラスタサイズ・最小サンプル数を掃引してクラスタ数を適正化）",
                    value=True, key="core_hdbscan_auto",
                ...
                if core_hdbscan_auto:
                    core_target_k = st.number_input(
                        ...
                        help="この数に近づくよう2パラメータを掃引します（母集団規模から自動初期化）。"
                             "結果のクラスタ数や粒度に満足できない場合は、この値を増減して「AIへの依頼文を作る」を押し直してください。"
                    ...
                    if st.session_state.get('core_hdbscan_auto_result'):
                        ...
                        _rv_txt = f"・分類品質（DBCV）={_rv:.2f}" if isinstance(_rv, (int, float)) else ""
                        st.info(
            ...
            if _kit_no_abstract:
                st.caption("※ このデータ（要約なし）では、依頼内容は「FIベースの技術分類・カテゴリ数は自動」に固定されます。")
                use_mece = True
            else:
            ...
            if not use_mece and not _kit_no_abstract:
            ...
            with st.spinner("クラスタリング & 代表文献を抽出中..."):
                try:
                    ...
                    if _core_si:
                        ...
                        _rv_txt = f"・分類品質（DBCV）={_rv:.2f}" if isinstance(_rv, (int, float)) else ""
                        st.info(f":material/smart_toy: 自動決定: 最小クラスタサイズ={_core_si['mcs']} / 最小サンプル数={_core_si['ms']} "
                                f"→ クラスタ数 {_core_si['k']}・ノイズ {_core_si['noise'] * 100:.1f}%{_rv_txt}")
                    cluster_ids = [c for c in sorted(set(int(l) for l in labels)) if c != -1]
                    if not cluster_ids:
                        st.error("有効なクラスタが得られませんでした。最小クラスタサイズを下げるか、"
                                 "クラスタ手法を「指定した数に分ける（KMeans・最適k自動）」に切り替えてお試しください。")
                        st.stop()
                    ...
                    # 分類コードをルールに使う設定なら、代表文献にコード（メイングループ）を付記する
                    _kit_use_code = (bool(st.session_state.get('core_search_include_ipc'))
                                     and 'ipc_main_group' in df_main.columns)
                    _code_label = utils.classification_code_label(col_map)

                    def _kit_doc_line(idx):
                        line = f"・{_core_text_preprocessor(texts_raw.iloc[idx])}"
                        if _kit_use_code:
                            _codes = df_main['ipc_main_group'].iloc[idx]
                            if isinstance(_codes, (list, tuple)) and len(_codes):
                                line += f"　[コード: {', '.join(sorted(str(c) for c in _codes)[:4])}]"
                        return line
                    ...
                    for ci in cluster_ids:
                        ...
                        sampled_docs.append(f"\n--- クラスタ {ci} （{int((labels == ci).sum())}件） ---\n" +
                            "\n".join([_kit_doc_line(idx) for idx in reps]))
                    ...
                    if _kit_no_abstract:
                        # 要約なしデータ: 名称だけでは語のヒットが乏しいため、FIコード主体の
                        # 技術分類1軸に絞って依頼する（課題・解決手段は名称からは設計できない）
                        instruction_text = (
                            "この特許母集団全体を網羅的に分類するための、**「技術分類」**の分類軸1つについて、**分類定義**（分類名、定義、分類論理式のセット）を設計してください。\n"
                            "\n# 重要: このデータの制約\n"
                            "- 各特許のテキストは発明の名称のみで、要約・請求項はありません。名称の語だけに頼ると取りこぼしが多くなります。\n"
                            "- そのため、**下の「FI頻出コード」を主役に**、FI（技術分類コード）で分類する論理式を設計してください。名称のキーワードは補助として組み合わせて構いません。\n"
                            "- 論理式にはFIコードをそのまま書けます（部分一致・半角英数で）。例: `H02J3/00 + H02J3/38`（メイングループ指定で配下の展開記号にも一致）、`H02J13`（さらに広い範囲の指定）。\n"
                            "- コードは `H02J3/00`（詰めた表記）でも `H02J 3/00`（公式表記）でも構いません。ただし**コード以外の語を空白で並べない**でください（`A B` は構文エラー。`A + B` か `A * B` と書く）。\n"
                            "- FIコードに near / adj は使わないでください（コード列は文章ではないため）。\n"
                            "- 課題分類・解決手段分類は、このデータでは設計できないため不要です。\n"
                            "\n# 重要: MECE (Mutually Exclusive, Collectively Exhaustive) の原則\n"
                            "- カテゴリは相互に排他的（ダブりがない）であり、かつ全体として網羅的（モレがない）であるように設計してください。\n"
                            "- カテゴリ数は、MECEを満たすのに最適だとあなたが判断する数（目安として5〜10個程度）にしてください。"
                        )
                    elif use_mece:
                        instruction_text = (
                            "この特許母集団全体を網羅的に分類するための、**「技術分類」「課題分類」「解決手段分類」**の3つの分類軸について、**分類定義**（分類名、定義、分類論理式のセット）を設計してください。\n"
                            "\n# 重要: MECE (Mutually Exclusive, Collectively Exhaustive) の原則\n"
                    ...
                    if _kit_no_abstract:
                        _json_format = (
                            ...
                            '      "name": "カテゴリ名 (例: 系統連系制御)",\n'
                            '      "definition": "カテゴリの定義...",\n'
                            '      "rule": "分類論理式 (例: H02J3/00 + H02J3/38 + (系統 * 連系))"\n'
                            '    },\n'
                            '    ...\n'
                            '  ]\n'
                            '}')
                        ...
                        _json_format = (
                            ...
                            '      "name": "カテゴリ名 (例: CO2分離膜)",\n'
                            '      "definition": "カテゴリの定義...",\n'
                            '      "rule": "分類論理式 (例: (CO2 + 二酸化炭素) * (膜 + メンブレン))"\n'
                            '    },\n'
                            '    ...\n'
                            '  ],\n'
                            '  "課題分類": [ ... ],\n'
                    ...
                    # 母集団の分類コード頻出（メイングループ）を依頼文に同梱する
                    # （要約なしデータでは必須の設計材料。要約ありでもコード使用ON時は補助材料）
                    _fi_section = ""
                    if (_kit_no_abstract or _kit_use_code) and 'ipc_main_group' in df_main.columns:
                        _fi_counts = df_main['ipc_main_group'].explode().dropna().astype(str)
                        _fi_counts = _fi_counts[_fi_counts.str.strip() != ''].value_counts().head(40 if _kit_no_abstract else 20)
                        if len(_fi_counts):
                            _fi_section = (
                                f"\n# {_code_label}頻出コード（この母集団のメイングループ上位）\n"
                                + "\n".join(f"- {c}: {n}件" for c, n in _fi_counts.items()) + "\n")

                    # 要約ありデータでコード使用ONのとき、依頼文にコード活用の節を足す
                    # （要約なしデータでは instruction_text に統合済みのため不要）
                    _code_rule_section = ""
                    if _kit_use_code and not _kit_no_abstract:
                        _code_rule_section = (
                            f"\n# 分類コード（{_code_label}）の活用\n"
                            f"- 分類論理式には、キーワードに加えて分類コードもそのまま書けます（部分一致・半角英数）。"
                            f"例: `H01M10/0562 + H01M10`（メイングループ指定で配下にも一致）。\n"
                            f"- コードは `H01M10/0562`（詰めた表記）でも `H01M 10/0562`（公式表記）でも構いません。"
                            f"どちらで書いてもデータ側の両方の表記に一致します。\n"
                            f"- 特に技術分類の軸では、コード+キーワードの組み合わせ"
                            f"（例: `H01M10/0562 + (固体電解質 * 電池)`）が取りこぼしに強く有効です。\n"
                            f"- コードに near / adj は使わないでください（コード列は文章の並びではないため）。\n"
                            f"- 各代表文献の末尾の [コード: ...] は、その文献に付与された分類コード（メイングループ）です。\n")
...
- **重要:** `near` や `adj` の条件の内部には `*` (AND) を含めることはできません（`+` (OR) は可能）。
    - OK: `(A + B) near10 C`
    - NG: `(A * B) near10 C`
- **重要（半角スペース）:** 半角スペースは語の区切りです。**語と語を演算子なしで並べないでください。**
    - NG: `固体電解質 電池`（間に演算子がない → 構文エラー）
    - OK: `固体電解質 * 電池` / `固体電解質 + 電池`
    - 例外: 分類コードの `H10B 41/27` のようなサブクラスとグループの間のスペースは1つのコードとして扱われます。
{_code_rule_section}
# 最重要ルール (キーワード拡張と表記ゆれ)
...
{sampled_docs_str}
"""
                    st.session_state['core_ai_prompt'] = prompt
                    st.toast("AIへの依頼文ができました。下の :material/counter_1: からコピーしてください。", icon=":material/smart_toy:")
                except Exception as e: st.error(f"エラー: {e}")

        # ---- 3ステップキット（生成済みの依頼文はここに残り続ける） ----
        if st.session_state.get('core_ai_prompt'):
            st.markdown("**:material/counter_1: AIへの依頼文をコピー**")
            st.code(st.session_state['core_ai_prompt'], language='markdown')
            st.caption(":material/touch_app: 枠の右上のコピーボタンで丸ごとコピーできます。作り直したいときは、もう一度「AIへの依頼文を作る」を押してください。")
            st.markdown("**:material/counter_2: 普段お使いのチャットAI（ChatGPT / Claude / Gemini 等）に貼り付けて送信**")
            st.caption("あなたの母集団に合わせた分類軸とルールのセットを、AIがJSON形式で返してくれます。APIキーは不要です。")
            st.markdown("**:material/counter_3: AIの回答をここに貼り戻して登録**")
            _kit_paste = st.text_area(
                "AIの回答（JSON）を貼り付け:", height=180, key="core_kit_paste",
                placeholder=('{\n  "技術分類": [ {"name": "…", "definition": "…", "rule": "H02J3/00 + (系統 * 連系)"} ]\n}'
                             if _kit_no_abstract else
                             '{\n  "技術分類": [ {"name": "…", "definition": "…", "rule": "(A + B) * C"} ],\n  "課題分類": [ … ],\n  "解決手段分類": [ … ]\n}'))
            if st.button(":material/download: ルールを登録して、そのまま分類まで実行", type="primary", key="core_kit_apply"):
                _n_rules = _import_rules_json(_kit_paste)
                if _n_rules:
                    st.success(f"{_n_rules} 個のルールを登録しました。続けて分類を実行します。")
                    _core_run_classification()
                    st.info("結果をマップで見るには、上の「フェーズ選択」で **④ 結果を見る（特許マップ）** を選んでください。", icon=":material/map:")
        else:
            st.caption("まずは上の「:material/counter_1: AIへの依頼文を作る」を押してください。特許の下ごしらえ（クラスタリング）と依頼文の生成を一度に行います。")

    # --- ② 分類ルールの登録・編集 ---
    elif current_phase.startswith("②"):
        st.subheader("② 分類ルールの登録・編集")
        ...
        with tab_manual:
            ...
            apollo_ui.howto(
                "<b>＋（OR）</b>: <code>耐久性 + 寿命</code> ＝ どちらかの語を含む ／ <b>＊（AND）</b>: <code>耐久性 * 向上</code> ＝ 両方の語を含む<br>"
                "<b>nearN（近傍）</b>: <code>A near10 B</code> ＝ AとBが10文字以内に近接（順序は不問） ／ <b>adjN（隣接）</b>: <code>A adj10 B</code> ＝ Aの後ろ10文字以内にB（順序どおり）<br>"
                "丸括弧 <code>( )</code> で優先順位を組めます（例: <code>(耐久性 + 寿命) * 向上</code>）。near / adj の内側に ＊(AND) は使えません（＋(OR) は可）。<br>"
                "<b>半角スペースは語の区切り</b>です。<code>固体電解質 電池</code> のように演算子なしで並べると構文エラーになります（<code>*</code> か <code>+</code> を挟んでください）。"
                "分類コードは <code>H10B41/27</code> でも <code>H10B 41/27</code> でも書けます（どちらもデータ側の両表記に一致します）。",
                title="論理式の書き方",
            )
        ...
        with tab_json:
            ...
            if st.button("JSONを一括インポート"):
                count = _import_rules_json(json_input)
    ...
    # --- ③ 分類の実行 ---
        ...
        _text_part_names = [n for k, n in (('title', '発明の名称'), ('abstract', '要約'), ('claim', '請求項'))
                            if col_map.get(k)]
        st.info("※ 探索範囲は「" + " + ".join(_text_part_names) + "」の結合テキストです。")
        _ipc_assigned = _render_ipc_search_checkbox()
        if not col_map.get('abstract') and _ipc_assigned:
            st.caption(":material/lightbulb: このデータには要約がないため、名称の語だけではヒットしにくくなります。FI（技術分類コード）を使ったルールがおすすめです。")

        if st.button("すべての分類を実行", type="primary"):
            if not st.session_state.core_classification_rules:
                st.error("ルールがありません。先に「② 分類ルールの登録・編集」でルールを登録してください。")
            else:
                # 実処理は共通関数へ（①のAIキット「登録して分類まで実行」と同一コードパス）
                _core_run_classification()

        st.markdown("---")
        st.subheader(":material/search: 未分類データの再分析 (『その他』を減らす)")
        if st.session_state.core_df_classified is not None:
            ...
            if rules:
                ...
                with rc1:
                    ...
                    re_feature_mode = 'sbert' if 'SBERT' in re_feature_label else 'tfidf'
                with rc2:
                    re_method_label = st.radio("クラスタ手法:",
                        ["数を自動選択（KMeans・k自動）", "数を手動指定（KMeans・k手動）", "数を自動で決める（HDBSCAN）"], key="re_cluster_method",
                            ...
                            with st.spinner(f"『その他』({len(others_df)}件) を分析中..."):
                                ...
                                if "HDBSCAN" in re_method_label:
                                    ...
                                    if re_k is None:  # k自動
                                        ...
                                        _use_k = int(_rk_r)
                                        st.caption(f"自動選択クラスタ数 k = {_use_k}（分類品質スコア最大）")
                                    else:  # k手動
...
"""
            ...
            if st.session_state.core_reanalyze_result:
                ...
                st.caption("返ってきたJSONは「② 分類ルールの登録・編集」の「JSON一括インポート」タブから追加登録できます。")

    # --- ④ 特許マップ作成 ---
    elif current_phase.startswith("④"):
        st.subheader("④ 特許マップ作成")

        if st.session_state.core_df_classified is None:
            st.warning("先に「③ 分類の実行」で分類を実行してください。")
        else:
            df_c = st.session_state.core_df_classified
            ...
            if st.session_state.get("core_phase4_run"):
                ...
                # 出願人カラムの区切り文字（Mission Control のユーザー設定。既定は ;）
                ...
                if '出願人' in [x_ax, y_ax]:
                    ...
                    st.markdown("##### :material/groups: 出願人軸の表示設定")
                ...
                if temp_df_g.empty:
                    ...
                    if col_map.get('applicant') in df_c.columns:
                        ...
                        top_apps = app_exploded.value_counts() # 全出願人
                    ...
                    def render_core_chart(sub_df, wrapper_key):
                        ...
                        if chart_type == "ヒートマップ":
                            ...
                            fig = px.scatter(
                                ...
                                size_max=40, color_discrete_sequence=apollo_ui.PALETTE,
                            ...
                            fig.update_xaxes(
                                ...
                                type='category',
                                showgrid=True, gridcolor='#e3e6ea', gridwidth=1, zeroline=False,
                                range=[-0.5, len(x_ord_global) - 0.5] # 全幅を確保
                            )
                            @st.fragment
                        ...
                        # --- 読み方カード ---
                        _dense_word = "色が濃いマス" if chart_type == "ヒートマップ" else "円が大きいマス"
                        apollo_ui.howto(
                            f"縦（{y_ax}）×横（{x_ax}）の各マスの数値は、<b>その組み合わせに該当する特許の件数</b>です。"
                            f"<b>{_dense_word}</b>＝件数が集中している注力領域、<b>0件や極端に少ないマス</b>＝空白領域（未開拓の狙い目か、そもそも成立しにくい組み合わせか）に注目してください。"
                            "1件の特許が複数のカテゴリに当てはまると重複して数えられるため、<b>マスの合計は母集団の件数と一致しない</b>ことがあります。"
                            "マスをクリックすると該当特許の一覧を確認できます。",
                            title="このマップの見方",
                        )
                        ...
                        utils.render_snapshot_button(
                            title=f"ルール分類マップ ({selected_app_label}): {x_ax} × {y_ax}",
                            description=f"ルール分類のクロス集計マトリクス: {x_ax} (X) × {y_ax} (Y)。対象: {selected_app_label}。",
...
各主張には必ず具体的な数値を1つ以上含めてください。""",
                            ...
                            constraints="下記「分類ルール（論理式）」でカテゴリの意味を正確に理解した上で分析すること。",
                    ...
                    try:
                        ...
                        st.caption(f":material/warning: WARN クラス分類マトリクス の CAPCOM 保存に失敗しました（要確認）: {e}")
