...
- [ ] **`flight_recorder.json` を読む**（アプリが自動記録した実際の分析条件。推定で書かないための正本）。**層ごとに扱いが違う**（JSON 内の `layer_policy` が正本）:
  - **層1 `lineage`（母集団の来歴）** — 読み込みファイルと件数・結合／重複除去の内訳・除外ワード数・要約の有無・分類コードが IPC か FI か。**本文で使ってよい**（母集団の定義章・信頼性注記）。`population_meta` と食い違う場合は `flight_recorder.json` が実測値なのでこちらを優先し、差異はユーザーに確認する
  - **層2 `runs[].judgment`（判断に効く条件）** — クラスタの粒度・クラスタ数・未分類率・品質指標（DBCV 等）・粒度の決め方。**「結論の前提と見直しのサイン」（→ `analysis/structured_techniques.md`・Check 31）の材料**として使う。⚠️ 数値をそのまま本文に並べるのではなく、「クラスタの粒度をこれ以上細かくすると〈クラスタ名〉は分割されうる」「未分類が N% あるため、この空白地帯の解釈は暫定である」のような**条件文に変換**して書く
  - **層3 `runs[].repro`（再現用パラメータ）** — 近傍数・最小距離・乱数シード・文埋め込みモデル名・所要秒数。**付録 A の表にだけ**転記する。**本文への数値記載は禁止**（内部パラメータの露出＝ Check 8 系に抵触）
...
- [ ] 特に注意すべきケース:
  ...
  - タイプ C では動態分析（出願人軸）の 4 象限は事実上意味がない（代替案を検討）
...
- [ ] `data_available = false` の場合、`AskUserQuestion` ツールで以下を提示:
  ```
  質問: 本セッションでは環境分析（学術論文・ニュース・政策の取り込み）が
...
- [ ] `AskUserQuestion` ツールで「各モジュールの Step 数・最低行数の理解(例: 俯瞰図分析 13セクション/250行)を一覧で提示し、これで進めて良いか」をユーザーに確認
- [ ] ユーザー応答を待つ
