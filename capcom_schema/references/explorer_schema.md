...
### クロスモジュール分析での活用
- **俯瞰図分析 / 探索的クラスタリング クラスタとの対応**: キーワード分析のコミュニティが、俯瞰図分析のクラスタや探索的クラスタリングの手動クラスタとどう対応するかを比較し、技術分類の妥当性を検証する
- **動態分析 4象限との組み合わせ**: 急上昇キーワードが動態分析のどの象限の出願人に多いかを分析し、成長領域と出願人の動態を関連づける
- **出願人・発明者ネットワーク ネットワークとの統合**: キーワード分析の技術キーワードクラスタと出願人・発明者ネットワークの人的ネットワーク（発明者コミュニティ）を突き合わせ、どの研究チームがどの技術領域を推進しているかを把握する
