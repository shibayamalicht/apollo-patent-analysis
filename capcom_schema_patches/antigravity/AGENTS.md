...
- **🔄 セッション・チェックポイント（各フェーズ境界で必須）**: 各フェーズの区切り（Phase A完了・Phase B完了・Phase Cの各モジュール完了ごと・Phase D着手前）で必ず、ゲート通過＋`_carryover.md` 更新の後に「新セッションに切り替えますか？」と提案して一旦停止する（続行/切替/`/compact` をユーザーが選ぶ）。詳細はスキル本体 `### 🔄 セッション・チェックポイント`
